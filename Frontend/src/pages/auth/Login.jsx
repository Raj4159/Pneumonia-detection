...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        const token = responseData.access_token;  // Extract the token from the response
        if (token) {
          cokkie.set('bearerToken', token);  // Save the token in cookies
      ...
      <div className="xl:mx-auto xl:w-full xl:max-w-sm 2xl:max-w-md">
        ...
        <form onSubmit={handleLogin} className="mt-8">
          <div className="space-y-5">
            ...
            {loginStatus === "error" && (
              <p className="text-red-600 text-center">Login failed. Please try again.</p>
            )}
          </div>
        </form>
      </div>
